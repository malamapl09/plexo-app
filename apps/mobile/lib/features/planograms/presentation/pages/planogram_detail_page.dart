...
class PlanogramDetailPage extends ConsumerStatefulWidget {
  final String submissionId;
  final PlanogramSubmission? submission;

  const PlanogramDetailPage({
    super.key,
  ...
  @override
  ConsumerState<PlanogramDetailPage> createState() =>
      _PlanogramDetailPageState();
}

class _PlanogramDetailPageState extends ConsumerState<PlanogramDetailPage> {
  PlanogramSubmission? _submission;
  ...
  Future<void> _handleResubmit() async {
    ...
    try {
      ...
      final success =
          await ref.read(planogramsProvider.notifier).resubmitPlanogram(
                widget.submissionId,
                photoUrls,
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Planograma reenviado exitosamente'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(error ?? 'Error al reenviar planograma'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (submission == null) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Detalle del Planograma'),
        ...
        child: Column(
          ...
          children: [
            ...
            if (submission.template != null) ...[
              ...
              if (submission.template!.description != null) ...[
            ...
            _buildInfoRow(
              ...
              label: 'Fotos',
              value: '${submission.photoUrls.length} foto${submission.photoUrls.length == 1 ? '' : 's'}',
              theme: theme,
            ),
            const SizedBox(height: 20),
            ...
            if (submission.photoUrls.isNotEmpty) ...[
              ...
              PhotoComparisonWidget(
                referencePhotos:
                    submission.template?.referencePhotoUrls ?? [],
                submittedPhotos: submission.photoUrls,
              ),
              const SizedBox(height: 24),
            ],

            // Resubmit section (only if NEEDS_REVISION)
              ...
              if (!_showResubmitForm) ...[
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Reenviar Planograma'),
                          ...
                          children: [
                            ...
                            Positioned(
                              ...
                              child: GestureDetector(
                                onTap: () => _removeResubmitPhoto(index),
                                child: Container(
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Notas sobre la correccion (opcional)...',
                    hintStyle:
  ...
  Widget _buildStatusBadge(PlanogramSubmission submission, ThemeData theme) {
    final colorScheme = theme.colorScheme;
